# CCF Import Flow

This repository is designed for Cloudera Cloud Factory repository import and catalog curation.

## HTTPS Helm Repository

CCF can consume a classic Helm repository published over HTTPS, the same way it consumes public repos like [CloudNativePG's Helm repo](https://cloudnative-pg.github.io/charts).

This repository now supports publishing a GitHub Pages-backed Helm repository with:

- packaged chart archives (`*.tgz`)
- `index.yaml` generated by `helm repo index`
- a lightweight `index.html` landing page at the repository root

Expected default URL:

- `https://<owner>.github.io/<repo>/`

If you configure the repository variable `HELM_REPO_URL`, the Pages workflow will use that URL when generating archive links in `index.yaml`.

## OCI Repositories

Import the curated OCI repository into CCF when you want OCI-backed consumption:

- curated wrappers: `oci://ghcr.io/<owner>/ccf-charts`

## Expected CCF Flow

1. Import the classic HTTPS Helm repository or OCI repository into CCF.
2. Create or select a catalog.
3. Bind curated applications into that catalog.
4. Bind projects to the catalog.
5. Install application instances into target namespaces.

The CCF MCP server exposes that same sequence through:

- `import-repository`
- `catalog-create`
- `catalog-app-add`
- `catalog-app-remove`
- `catalog-app-defaults-set`
- `bind-projects-to-catalog`
- `app-install`

## Curated Defaults

Curated wrapper charts carry two configuration layers:

- `values.yaml`: safe default values intended to install on a typical CCF Kubernetes project
- `questions.yaml`: operator-facing prompts for the values most likely to vary between organizations

Typical curated questions cover:

- ingress enablement, class, and hostname
- service type
- storage class and PVC size
- lightweight resource settings
- bootstrap admin or secret references when the upstream chart supports them

Generated catalog prompts are intended to map directly to CCF app parameters. Boolean and integer
defaults should remain available as questions, but validation tooling may need to normalize their
serialized values before sending them through the CCF install path.

## Catalog Strategy

Recommended approach:

- one curated catalog sourced from `ccf-charts` or the GitHub Pages Helm repository
- add any non-curated community repositories directly to CCF from their upstream endpoints

That keeps the repository focused on curated defaults while still letting CCF consume external
community charts directly when needed.

## Notes On Coverage

- `rabbitmq` and `wordpress` are not packaged here; add their upstream community repositories
  directly to CCF when you need them
- `memcached` is still not supported because no maintained non-Bitnami source was validated
- some curated upstream charts still require manual review because their upstream ecosystem references Bitnami helper dependencies even when the deployed images are not Bitnami-based
